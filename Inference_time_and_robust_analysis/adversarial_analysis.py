"""
Adversarial Scenario Analysis Experiment
Response to reviewer question: How does NS-FirmID handle banners that are intentionally modified to deceive firmware version identification
"""
...
def analyze_misleading_scenarios(df, banners):
    print("="*100)
    print("Adversarial Scenario Analysis: Misleading Version Information")
    print("="*100)
    ...
    print("\n1. Version-like String Statistics by Result Type")
    print("-"*100)
    print(f"{'Result Type':<10} {'Sample Count':<10} {'Avg IP':<15} {'Avg Version':<15} {'Avg Date':<15} {'Avg Total':<15}")
    print("-"*100)
    ...
    print("\n2. High Interference Scenario Analysis (Version-like Strings > 10)")
    print("-"*100)
    ...
    print(f"Total high interference samples: {total_high}")
    ...
    print("\n3. FP Case Analysis (Potential Adversarial Scenarios)")
    print("-"*100)
    
    fp_samples = results['FP']
    fp_with_multiple_versions = [s for s in fp_samples if s['counts']['version_numbers'] > 3]
    
    print(f"FP samples with multiple version numbers: {len(fp_with_multiple_versions)}/{len(fp_samples)}")
    
    for s in fp_with_multiple_versions[:5]:
        print(f"\n  Sample: {s['sample_id']}")
        print(f"  True Label: {s['label']}")
        print(f"  Version-like String Statistics: {s['counts']}")
    
    return results

def analyze_confidence_filtering(df, banners):
    print("\n" + "="*100)
    print("Confidence Filtering Mechanism Analysis")
    print("="*100)
    ...
    print(f"\n{'Metric':<15} {'Raw Output':<15} {'After Filter':<15} {'Improvement':<15}")
    print("-"*60)
    ...
    print(f"\n{'FP Reduction':<15} {metrics_raw['FP']:<15} {metrics_filtered['FP']:<15} {metrics_raw['FP'] - metrics_filtered['FP']:<15}")
    
    return metrics_filtered, metrics_raw

def analyze_validator_effectiveness(df, banners):
    print("\n" + "="*100)
    print("Validator Agent Effectiveness Analysis")
    print("="*100)
    ...
    print(f"FP Analysis:")
    print(f"  - FP with version in banner but extracted incorrectly: {fp_with_version_in_banner}")
    print(f"  - FP with version not in banner (hallucination): {fp_without_version_in_banner}")
    
    print(f"\nTN Analysis:")
    print(f"  - Correctly identified samples without version: {tn_correct}/{tn_total}")
...
def generate_response_summary():
    print("\n" + "="*100)
    print("Reviewer Question Response Summary")
    print("="*100)
